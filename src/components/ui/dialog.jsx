  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      onPointerDownOutside={(e) => {
        ...
        if (target?.closest('[data-radix-collection-item]')?.closest('[role="dialog"]')) {
           e.preventDefault();
        }
      }}
      className={cn(
        // Responsive width and positioning
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-[95vw] sm:max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200",
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]",
        "rounded-lg", // Ensure rounded corners
        className
      )}
      {...props}>
      {children}
...
const DialogFooter = ({
  ...
  <div
    className={cn("flex flex-col-reverse gap-2 sm:gap-0 sm:flex-row sm:justify-end sm:space-x-2 mt-4 sm:mt-0", className)}
    {...props} />
)
